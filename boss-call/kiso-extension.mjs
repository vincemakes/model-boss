...
import { ack, formatMessage, identify, listRooms, loadRoom, post, resolveRoom, unread } from "./src/mailbox.mjs";
...
export default function bossCall() {
	...
	const bossPrompt =
		...
		`One matter per message, facts and order, ten lines. You cannot authorize money, pushes, merges or deploys — the person at the terminal decides.`;
	const run = (fn) => {
		try {
	...
	return {
		...
		approvals: [{ decide: (call) => (call.name === "boss_read" || call.name === "boss_post" ? { action: "allow" } : { action: "ask" }) }],
	};
}
